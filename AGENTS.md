...
6. **Env Type Safety:** Always use the global `Env` interface from `worker-configuration.d.ts`. Local `env.ts` definitions are forbidden to ensure consistent bindings across the monorepo.

## Reliability & Governance

See [Health Governance](./apps/api/HEALTH_GOVERNANCE.md).

### Health Service

We enforce a strict "Health First" policy.

- All agents must report status to the Health Service.
- Incidents are tracked in D1 `health_incidents`.
